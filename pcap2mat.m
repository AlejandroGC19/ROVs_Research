...
function capture = pcap2mat(filter, decodeas_and_dissector, filename_or_interface, capture_stop_criteria)
% pcap2matlab() imports network protocol analyzer capabilties into MATLAB.
%   
%   capture = pcap2mat(filter, decodeas_and_dissector,filename_or_interface, capture_stop_criteria) 
...
%     end
    ...
    while (~feof(fid))%~isempty(line{1})
        ...
        for srclineidx = 1:5:length(linestr{1}),
            ...
            capture(dstlineidx).frametime = strcat(line(1:2), line(4:5) , line(7:8) , line(10:12) , line(14:16));
            ...
            for idx=1:min(SizeofDissector,Sizeof_main_parser),
                switch (FieldsofDissector{idx})
                    ...
                    case {'frametime'}
                        ...
                        capture(dstlineidx).frametime = strcat(datetime_parser{1}{2}(1:2),'-',datetime_parser{1}{1}(1:3),'-',datetime_parser{1}{3}(1:4),'-',datetime_parser{1}{4}(1:2),':',datetime_parser{1}{4}(4:5),':',datetime_parser{1}{4}(7:end))
                         
                    otherwise
                        if isempty(main_parser{1}{idx})
                            capture(dstlineidx).(FieldsofDissector{idx}) = 0;
                        elseif ~isempty(regexp(main_parser{1}{idx},'0x','ONCE'))
                            capture(dstlineidx).(FieldsofDissector{idx}) = main_parser{1}{idx}(3:end);
                        else
